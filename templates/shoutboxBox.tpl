...
<div class="border titleBarPanel shoutboxBox">
	...
	<div class="container-1" id="{@$box->getStatusVariable()}">
		<div class="containerContent">
			...
			<div class="border infoBox container-1">
				<div class="containerContent">
					<div id="shoutboxContent">
						{assign var=sortedShoutboxEntries
						value=$box->entries|array_reverse} {foreach
						from=$sortedShoutboxEntries item=entry}
						<p id="shoutboxEntry{@$entry->entryID}">
							<span class="light">[{@$entry->time|shorttime}]</span> {if
							$entry->isDeletable()} <a
								...
								style="font-weight: bold;">{@$entry->getWhisperPrefix()}</span>

							<a href="#" style="text-decoration: none;">{@$entry->getStyledUsername()}</a>
							{if $entry->me = 0} : {@$entry->getFormattedMessage()}
						</p>
			...
			{if $portalMode != 'edit' &&
			$this->user->getPermission('user.shoutbox.canAddEntry')}
			<form method="post" action="index.php?action=ShoutboxEntryAdd"
				id="shoutboxEntryAddForm">
				<div
					id="shoutboxFormLayout-{if $this->user->userID == 0}2{else}1{/if}"
					style="width: 95%;">
					<a href="javascript: toggle('shoutboxSmileyContainer')"
						style="text-decoration: none;"> <img
						src="{@RELATIVE_WCF_DIR}icon/dropDownMenuS.png" alt="smilie"
						name="smilie" />
					</a> {if $this->user->userID == 0} <input type="text"
					...
					{/if} <input type="text" class="inputText" name="message"
						id="shoutboxMessage" value="" /> <input type="image"
						class="inputImage" id="shoutboxSubmit"
						src="{icon}submitS.png{/icon}" /> {@SID_INPUT_TAG}
				</div>
			</form>
			{if SHOUTBOX_ENABLE_SMILEY_LIST}
			<div class="" id="shoutboxSmileyContainer"
	...
	//]]>
				...
				{/if} {if $this->user->getPermission('user.shoutbox.bbcode.b')} <img
				...
				$this->user->getPermission('user.shoutbox.bbcode.center')} <img
					...
					onclick="shoutbox.insertBBCode('[align=center]','[\/align]');"
					alt="" /> {/if} {if
				$this->user->getPermission('user.shoutbox.bbcode.align')} <img
					src="wcf/icon/wysiwyg/textAlignRightM.png"
					onclick="shoutbox.insertBBCode('[align=right]','[\/align]');"
					alt="" /> {/if} {if
				$this->user->getPermission('user.shoutbox.bbcode.size')} <select
					name="fontsize" id="shoutboxFontsize" style="height: 20px;"
					onchange="shoutbox.insertBBCode('[size=' + this.value + ']','[\/size]', 'shoutboxFontsize')">
				...
				$this->user->getPermission('user.shoutbox.bbcode.font')} <select
					name="font" id='shoutboxFontface' style="height: 20px;"
					onchange="shoutbox.insertBBCode('[font=' + this.value + ']','[\/font]', 'shoutboxFontface')">
					...
					<option value="courier new" style="font-family: Courier New;">Courier
						New</option>
				...
				$this->user->getPermission('user.shoutbox.bbcode.color')} <select
					name="fontcolor" id="shoutboxFontcolor" style="height: 20px;"
					onchange="shoutbox.insertBBCode('[color=' + this.value + ']','[\/color]','shoutboxFontcolor')">
				...
				$this->user->getPermission('user.shoutbox.bbcode.quote')} <img
					...
					onclick="shoutbox.insertBBCode('[quote]','[\/quote]');" alt="" />
				{/if} {if $this->user->getPermission('user.shoutbox.bbcode.code')} <img
					src="wcf/icon/wysiwyg/insertCodeM.png"
					onclick="shoutbox.insertBBCode('[code]','[\/code]');" alt="" />
				{/if} {if $this->user->getPermission('user.shoutbox.bbcode.css')} <img
					src="wcf/icon/wysiwyg/insertCssM.png"
					onclick="shoutbox.insertBBCode('[css]','[\/css]');" alt="" /> {/if}
				{if $this->user->getPermission('user.shoutbox.bbcode.html')} <img
					src="wcf/icon/wysiwyg/insertHtmlM.png"
					onclick="shoutbox.insertBBCode('[html]','[\/html]');" alt="" />
				{/if} {if $this->user->getPermission('user.shoutbox.bbcode.java')} <img
					src="wcf/icon/wysiwyg/insertJavaM.png"
					onclick="shoutbox.insertBBCode('[java]','[\/java]');" alt="" />
				{/if} {if $this->user->getPermission('user.shoutbox.bbcode.js')} <img
					src="wcf/icon/wysiwyg/insertJavaScriptM.png"
					onclick="shoutbox.insertBBCode('[js]','[\/js]');" alt="" /> {/if}
				{if $this->user->getPermission('user.shoutbox.bbcode.php')} <img
					src="wcf/icon/wysiwyg/insertPhpM.png"
					onclick="shoutbox.insertBBCode('[php]','[\/php]');" alt="" /> {/if}
				{if $this->user->getPermission('user.shoutbox.bbcode.mysql')} <img
					src="wcf/icon/wysiwyg/insertMysqlM.png"
					onclick="shoutbox.insertBBCode('[mysql]','[\/mysql]');" alt="" />
				{/if} {if $this->user->getPermission('user.shoutbox.c.java')} <img
					src="wcf/icon/wysiwyg/insertCM.png"
					onclick="shoutbox.insertBBCode('[c]','[/c]');" alt="" /> {/if}
				<ul class="smileys">
					...
					<li><img onmouseover="this.style.cursor='pointer'"
						...
						title="{lang}{$smiley->smileyTitle}{/lang}" /></li> {/foreach}
				</ul>
			</div>
			{/if} {/if}
		</div>
