...
<div class="containerIcon"><a href="javascript: void(0)"
...
<div class="containerContent"><script type="text/javascript"
	src="{@RELATIVE_WCF_DIR}js/Shoutbox.class.js"></script> <script
...
<div class="containerContent">
<div id="shoutboxContent">{assign var=sortedShoutboxEntries
value=$box->entries|array_reverse} {foreach from=$sortedShoutboxEntries
item=entry}
...
{/if} <input type="text" class="inputText" name="message"
	id="shoutboxMessage" value="" /> <input type="image" class="inputImage"
	id="shoutboxSubmit" src="{icon}submitS.png{/icon}" /> {@SID_INPUT_TAG}
</div>
</form>
...
<ul class="smileys">
	...
	<li><img onmouseover="this.style.cursor='pointer'"
		...
		title="{lang}{$smiley->smileyTitle}{/lang}" /></li>
	{/foreach}
</ul>
